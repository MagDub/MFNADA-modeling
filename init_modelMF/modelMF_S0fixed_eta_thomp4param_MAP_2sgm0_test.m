function [nMAP, nLogL, mo, logL] = modelMF_S0fixed_eta_thomp4param_MAP_2sgm0_test(param_val,param_names,sID,settings,data,gameIDs, prior)
...
%         disp([int2str(c) '_' int2str(g)])
        ...
        if g_idx < g && mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g)==mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g_idx)
            ...
            % compute model game
            [logL(c,g),mo] = model_game_MF_2sgm0_test(mo,tmp_dat,c,g,settings.opts.TLT);
        end
    end
end
