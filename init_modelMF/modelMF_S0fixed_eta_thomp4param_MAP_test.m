function [nMAP, nLogL, mo, logL] = modelMF_S0fixed_eta_thomp4param_MAP_test(param_val,param_names,sID,settings,data,gameIDs, prior)
...
%% initialise model
mo = initialise_model_MF_S0fixed_eta_2sgm0_test2(settings);

disp(strcat('after initialise, sigma_0(1)=', 32, num2str(mo.params.sgm0(1))))
disp(strcat('after initialise, sigma_02(1)=', 32, num2str(mo.params.sgm02)))

%% fill in parameters, model-funs etc
mo = prep_model_MF(mo,settings,param_val,param_names);

disp(strcat('after prep=_model, sigma_0(1)=', 32, num2str(mo.params.sgm0(1))))
disp(strcat('after prep=_model, sigma_02(1)=', 32, num2str(mo.params.sgm02(1))))
...
%         disp([int2str(c) '_' int2str(g)])
        ...
        if g_idx < g && mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g)==mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g_idx)
            ...
            % compute model game
            [logL(c,g),mo] = model_game_MF_test(mo,tmp_dat,c,g,settings.opts.TLT);
        end
    end
end
