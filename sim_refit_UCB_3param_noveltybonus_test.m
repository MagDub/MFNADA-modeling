function sim_refit_UCB_3param_noveltybonus_test(ID)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    settings.funs.learningfun   = @kalman_filt_new;
    settings.desc = ['sim_UCB_noveltybonus' int2str(ID)];    % description of model (settings, etc)
    settings.params.param_names = {'gamma',   'tau',    'eta', 'xi'};   % is same param name as prev, write ''
    settings.params.lb          = [param_bounds_gamma(1)    param_bounds_tau(1) param_bounds_eta(1) param_bounds_xi(1)];    % lower bound
    settings.params.ub          = [param_bounds_gamma(2)    param_bounds_tau(2) param_bounds_eta(2) param_bounds_xi(2)];    % upper bound
    
    if holly == 1
        sim_dir = strcat('/home/mdubois/data/simulation_data/UCB_noveltybonus/4params/', int2str(n_per_dim), 'perdim/');
    else
        sim_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\UCB_noveltybonus\4params\', int2str(n_per_dim), 'perdim\');
    end

    %% load parameter values
    tmp = load([sim_dir 'inp_params_UCB_noveltybonus.mat']);
    ...
    for c = 1:settings.task.N_hor % 1
        for g = 1:settings.task.Ngames_per_hor % 200   
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/simulation_data/UCB_noveltybonus/4params/', int2str(n_per_dim), 'perdim/results/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\UCB_noveltybonus\4params\', int2str(n_per_dim), 'perdim\results\');
    end
