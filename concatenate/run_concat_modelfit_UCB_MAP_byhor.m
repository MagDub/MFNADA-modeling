...
[all_parameters_UCB] = concatenate_all_params_UCB_MAP_byhor(num_part);
...
% % remove the xi outlier
% disp('outlier number ');
% disp(size(find(xi_mat_UCB(:,1)>0.49)));
% 
% gamma_mat_UCB(find(xi_mat_UCB(:,1)>0.49),:)=[];
% tau_mat_UCB(find(xi_mat_UCB(:,1)>0.49),:)=[];
% xi_mat_UCB(find(xi_mat_UCB(:,1)>0.49),:)=[];
