function fit_UCBargmax_weighted_4param_KFsigma(ID, optim_algo)
 
holly = 0;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
param_bounds_gamma = [10^-4,10]; % information bonus
param_bounds_tau = [10^-4,10]; % inverse temperature
param_bounds_xi = [10^-4,0.5]; % epsilon greedy
param_bounds_weight = [0,1];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    algo = 'UCBargmaxweightedKFsigma';
    n_param = 4;
    ...
    settings.funs.learningfun   = @argmax_weighted; % TO CHANGE
    settings.desc = ['UCBargmaxweightedKFsigma'];    % description of model (settings, etc)
    settings.params.param_names = {'gamma',     '',              'tau',    '',     'xi', '', 'w_argmax', ''};   % is same param name as prev, write ''
    settings.params.lb          = [param_bounds_gamma(1) param_bounds_gamma(1)   param_bounds_tau(1)  param_bounds_tau(1) param_bounds_xi(1) param_bounds_xi(1) param_bounds_weight(1) param_bounds_weight(1)];    % lower bound
    settings.params.ub          = [param_bounds_gamma(2) param_bounds_gamma(2)   param_bounds_tau(2)  param_bounds_tau(2)  param_bounds_xi(2) param_bounds_xi(2) param_bounds_weight(2) param_bounds_weight(2)];    % upper bound
    ...
    modelfun = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse_argmax_w(x,settings.params.param_names,ID,settings,data,gameIDs);   
...
%      modelfun_LL = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse_LL(x,settings.params.param_names,ID,settings,data,gameIDs);  
