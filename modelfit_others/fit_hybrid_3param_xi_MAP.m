function fit_hybrid_3param_xi_MAP(ID, optim_algo)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/participant_data/',algo,'/',int2str(n_param), 'params_xi/results/', optim_algo,'/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\',algo,'\', int2str(n_param), 'params_xi\results\',optim_algo,'\');
    end
...
%   
    ...
    modelfunLL = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse(x,settings.params.param_names,ID,settings,data,gameIDs);
    modelfun = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse_MAP_hybrid(x,settings.params.param_names,ID,settings,data,gameIDs);
