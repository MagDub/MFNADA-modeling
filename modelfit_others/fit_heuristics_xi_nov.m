function fit_heuristics_xi_nov(ID, optim_algo)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/participant_data/',algo,'/',int2str(n_param), 'params_xi_nov/results/', optim_algo,'/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\',algo,'\', int2str(n_param), 'params_xi_nov\results\',optim_algo,'\');
    end
    ...
    modelfun = @(x) modelMF_S0fixed_sgm0fixed_eta(x,settings.params.param_names,ID,settings,data,gameIDs); 
