function fit_UCBargmax_3param_v3(ID, optim_algo)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    settings.opts.TLT       = [];
    settings.funs.decfun        = @softmax_argmax;
    settings.funs.valuefun      = @UCB; 
    settings.funs.priorfun      = [];
    settings.funs.learningfun   = @kalman_filt;
    settings.desc = ['UCBargmaxv3'];    % description of model (settings, etc)
    settings.params.param_names = {'gamma'   ''         'tau'      ''    'xi'  ''   'beta1'   'beta2'};   % is same param name as prev, write ''
    settings.params.lb          = [param_bounds_gamma(1)   param_bounds_gamma(1) param_bounds_tau(1) param_bounds_tau(1) param_bounds_xi(1) param_bounds_xi(1) param_bounds_beta(1) param_bounds_beta(1)];    % lower bound
    settings.params.ub          = [param_bounds_gamma(2)   param_bounds_gamma(2) param_bounds_tau(2) param_bounds_tau(2) param_bounds_xi(2) param_bounds_xi(2) param_bounds_beta(2) param_bounds_beta(2)];    % upper bound
    ...
    modelfun = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse_beta(x,settings.params.param_names,ID,settings,data,gameIDs);   
