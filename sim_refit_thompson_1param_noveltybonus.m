function sim_refit_thompson_1param_noveltybonus(ID)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    settings.funs.valuefun      = @mvnorm_Thompson_noveltybonus; %@hybrid; % @thompson; % 
    settings.funs.priorfun      = [];
    settings.funs.learningfun   = @kalman_filt;
    settings.desc = ['sim_thompson_noveltybonus' int2str(ID)];    % description of model (settings, etc)
    settings.params.param_names = {'xi', 'eta'};   % is same param name as prev, write ''
    settings.params.lb          = [param_bounds_xi(1) param_bounds_eta(1)];    % lower bound
    settings.params.ub          = [param_bounds_xi(2) param_bounds_eta(2)];    % upper bound
    
    if holly == 1
        sim_dir = strcat('/home/mdubois/data/simulation_data/thompson/2params/', int2str(n_per_dim), 'perdim/');
    else
        sim_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\thompson\2params\', int2str(n_per_dim), 'perdim\');
    end
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/simulation_data/thompson/2params/', int2str(n_per_dim), 'perdim/results/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\thompson\2params\', int2str(n_per_dim), 'perdim\results\');
    end
