% function sim_refit_thompson_3param_noveltybonus_parhor_MAP_test(ID)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if holly == 1
        sim_dir = strcat('/home/mdubois/data/simulation_data/thompson/parhor_MAP/', int2str(n_per_dim), 'perdim_newbounds_sgm0/');
    else
    ...
    %% initialise model
    mo = initialise_model_MF_S0fixed_eta_2sgm0(settings); %CHANGED
        ...
        for g = 1:settings.task.Ngames_per_hor         
            ...
            for t = 1:size(tmp_dat.alltrees,1)+1 %CHANGED
                if t == 1 % plug in priors
                    ...
                    mo.mat.sgm{c,g}(:,t) = mo.params.sgm0(c); %CHANGED
                end
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/simulation_data/thompson/parhor_MAP/', int2str(n_per_dim), 'perdim/results/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\thompson\parhor_MAP\', int2str(n_per_dim), 'perdim\results\');
    end
...
%     save_sim(results_dir, ID, settings, data, gameIDs, para_vals, mo);
    ...
    %save_func_MAP(ID, settings, results_dir, mEparams, mEmle, mEexitflag, mEsubj, mEMAP);   
