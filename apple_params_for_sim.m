function [user,params] = apple_params_for_sim(ID,params)
...
n_cond = 2;
items_per_cond = params.task.exp.n_trialPB; %100    % repeat each twice (to assess randomness) -> will be doubled

% if items_per_cond * n_cond * 2 ~= params.task.exp.n_blocks * params.task.exp.n_trialPB
%     error('condition and item numbers do not match')
% end
...
elseif (user.unused_tree(trial) == 1)
    ...
    if ~isempty(find(user.item(1,trial).initial_apples.tree==4,1)) %trials where there is a D tree
        ind_tree_D = find(user.item(1,trial).initial_apples.tree == 4,1);
        minimas = find(user.item(1,trial).initial_apples.size == min(user.item(1,trial).initial_apples.size));
        tree_minimas = user.item(1,trial).initial_apples.tree(minimas);
            while size(minimas,2)~=1 || (size(minimas,2)==1 && tree_minimas~=4)
                user.item(1,trial).initial_apples.size(ind_tree_D) = user.item(1,trial).initial_apples.size(ind_tree_D)-1;
                minimas = find(user.item(1,trial).initial_apples.size == min(user.item(1,trial).initial_apples.size));
                tree_minimas = user.item(1,trial).initial_apples.tree(minimas);
            end
            
        if user.item(1,trial).initial_apples.size(ind_tree_D)<2 %make sure that it is not smaller than 2
            user.item(1,trial).initial_apples.size(ind_tree_D) = 2;
        end
    end
...
%% prepare logfile
user.log_desc = {'Block','Blocktrial','Horizon','GameID','Trial','TreeA','TreeB','TreeC','TreeD','Size','RT', 'PressedKey', 'UnusedTree'};
user.log = [];
