function cv_mod15_heuristics(ID)
    ...
    for k = 1:Kf
        ...
        algo = 'crossval_heuristics';
        n_param = 2;

        if holly == 1
            results_dir = strcat('/home/mdubois/data/dev_participant_data/',algo,'/',int2str(n_param), 'params_eta/results/');
        else
            results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\k',int2str(Kf),'\',algo,'\', int2str(n_param), 'params_eta\results\');
        end
        ...
        settings.desc = ['heuristics'];    % description of model (settings, etc)
        ...
        modelfun = @(x) modelMF_S0fixed_sgm0fixed_crossval_nolapse_eta(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);  
        ...
        [test_nLogL, ~, ~] = modelMF_S0fixed_sgm0fixed_crossval_nolapse_eta(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, trials_tested_on);
