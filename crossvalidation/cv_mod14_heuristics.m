function cv_mod14_heuristics(ID)
    ...
    for k = 1:Kf
        ...
        if holly == 1
            ...
            results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\k',int2str(Kf),'\',algo,'\', int2str(n_param), 'params\results\');
        end
        ...
        modelfun = @(x) modelMF_S0fixed_sgm0fixed_crossval_nolapse(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);  
        ...
        [test_nLogL, ~, ~] = modelMF_S0fixed_sgm0fixed_crossval_nolapse(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, trials_tested_on);
