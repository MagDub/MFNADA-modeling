...
function likelihood_space_UCB_const_tau(ID)
%for ID = 502:560
    n_per_dim = 25;
    ...
    tau_both=sort([fmincon_data_tau genetic_data_tau]);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    param_bounds_gamma = [0.01,5]; % information bonus
    param_bounds_tau = tau_both; % inverse temperature
    ...
    for i_tau=1:size(vec_tau,2)
        for i_gamma=1:size(vec_gamma,2)
            for i_xi=1:size(vec_xi,2)
                ...
                nlogLmat_cst_tau(i_xi,i_gamma, i_tau) = nLogL; 
            end
        end
    end
   
    struct_cst_tau.nlogLmat = nlogLmat_cst_tau;
    struct_cst_tau.param_bounds_gamma = param_bounds_gamma;
    struct_cst_tau.param_bounds_tau = param_bounds_tau;
    struct_cst_tau.param_bounds_xi = param_bounds_xi;
    struct_cst_tau.n_per_dim = n_per_dim;
    struct_cst_tau.fmincon_data_gamma = fmincon_data_gamma;
    struct_cst_tau.fmincon_data_tau = fmincon_data_tau;
    struct_cst_tau.fmincon_data_xi = fmincon_data_xi;
    struct_cst_tau.genetic_data_gamma = genetic_data_gamma;
    struct_cst_tau.genetic_data_tau = genetic_data_tau;
    struct_cst_tau.genetic_data_xi = genetic_data_xi;
    
    if holly == 0
        save(strcat('D:\MaggiesFarm\modeling_28_02\participant_data\LLsurfaces\UCB\participant_', int2str(ID), '_struct_cst_tau.mat'),'-struct', 'struct_cst_tau');
    elseif holly == 1
        save(strcat('/home/mdubois/data/participant_data/LLsurfaces/UCB/participant_', int2str(ID), '_struct_cst_tau.mat'),'-struct', 'struct_cst_tau');
    end
