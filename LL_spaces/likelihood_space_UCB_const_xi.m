...
function likelihood_space_UCB_const_xi(ID)
%for ID = 502:560
    n_per_dim = 25;
    ...
    param_bounds_xi = xi_both; % epsilon greedy
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i_tau=1:size(vec_tau,2)
        for i_gamma=1:size(vec_gamma,2)
            for i_xi=1:size(vec_xi,2)
                ...
                nlogLmat_cst_xi(i_tau, i_gamma, i_xi) = nLogL; 
            end
        end
    end
   
    struct_cst_xi.nlogLmat = nlogLmat_cst_xi;
    struct_cst_xi.param_bounds_gamma = param_bounds_gamma;
    struct_cst_xi.param_bounds_tau = param_bounds_tau;
    struct_cst_xi.param_bounds_xi = param_bounds_xi;
    struct_cst_xi.n_per_dim = n_per_dim;
    struct_cst_xi.fmincon_data_gamma = fmincon_data_gamma;
    struct_cst_xi.fmincon_data_tau = fmincon_data_tau;
    struct_cst_xi.fmincon_data_xi = fmincon_data_xi;
    struct_cst_xi.genetic_data_gamma = genetic_data_gamma;
    struct_cst_xi.genetic_data_tau = genetic_data_tau;
    struct_cst_xi.genetic_data_xi = genetic_data_xi;
    
    if holly == 0
        save(strcat('D:\MaggiesFarm\modeling_28_02\participant_data\LLsurfaces\UCB\participant_', int2str(ID), '_struct_cst_xi.mat'),'-struct', 'struct_cst_xi');
    elseif holly == 1
        save(strcat('/home/mdubois/data/participant_data/LLsurfaces/UCB/participant_', int2str(ID), '_struct_cst_xi.mat'),'-struct', 'struct_cst_xi');
    end
