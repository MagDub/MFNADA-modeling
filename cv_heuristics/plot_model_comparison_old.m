...
mod.model_type{model} = 'heuristics';
mod.name{model} = 'heuristics';
mod.legend{model} = 'softmax + \epsilon';
%%%%%
% 19
model = model + 1;
...
mod.model_type{model} = 'heuristics';
mod.name{model} = 'heuristics';
mod.legend{model} = 'softmax + \eta + \epsilon'; 

%% Plotting  
average_prob_mat = [];
mean_all=nan(1,20);
mean_all_pp=nan(length(participant_list),20);
stderror_all=nan(1,20);
number_par_all=zeros(1,20);
...
col_(1,:) = [0.9252090200519562 0.839215695858002 0.839215695858002];
col_(2,:) = [0.5820313750267029 0.3882353007793203 0.3882353007793203];
col_(3,:) = [0.6720509823322296 0.20235292020322708 0.20235292020322708];
...
% % Export
% addpath('../../export_fig')
% export_fig(['./Fig_model_comparison.png'],'-nocrop','-r200')