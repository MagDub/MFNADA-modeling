...
for k = 1%:Kf
    ...
    algo = 'mod_20'; % V=0 for all -> softmax       
    ...
    if ~exist(results_dir)
        ...
        settings.funs.valuefun          = @nullvalue_eta;
        settings.funs.priorfun          = [];
        settings.funs.learningfun       = @kalman_filt;
        settings.desc                   = ['mod20'];    
        settings.params.param_names     = {'Q0'   'xi'  ''  'eta'   '' 'tau' ''};   
        settings.params.lb              = [param_bounds_Q0(1)  param_bounds_xi(1)   param_bounds_xi(1)  param_bounds_eta(1) param_bounds_eta(1) param_bounds_tau(1) param_bounds_tau(1)];    
        settings.params.ub              = [param_bounds_Q0(2)  param_bounds_xi(2)   param_bounds_xi(2)  param_bounds_eta(2) param_bounds_eta(2) param_bounds_tau(2) param_bounds_tau(2)];       
    ...
    [test_nLogL, ~, ~] = modelMF_S0fixed_sgm0fixed_crossval_eta_test(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, trials_tested_on);
