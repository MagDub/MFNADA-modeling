function sim_refit_UCBmean_3param(ID)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    settings.funs.learningfun   = @simple_mean;
    settings.desc = ['sim_UCBmean_' int2str(ID)];    % description of model (settings, etc)
    ...
    if holly == 1
        sim_dir = strcat('/home/mdubois/data/simulation_data/UCBmean/3params/', int2str(n_per_dim), 'perdim/');
    else
        sim_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\UCBmean\3params\', int2str(n_per_dim), 'perdim\');
    end

    %% load parameter values
    tmp = load([sim_dir 'inp_params_UCBmean.mat']);
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/simulation_data/UCBmean/3params/', int2str(n_per_dim), 'perdim/results/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\UCBmean\3params\', int2str(n_per_dim), 'perdim\results\');
    end
    ...
    modelfun = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse_simple(x,settings.params.param_names,ID,settings,data,gameIDs);   
