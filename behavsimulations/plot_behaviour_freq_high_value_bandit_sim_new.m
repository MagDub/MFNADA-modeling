...
exploit_SH_perc_nosim = (chosenOption.ABD.freq(:,1)+chosenOption.AB.freq(:,1)+chosenOption.BD.freq(:,1)+chosenOption.AD.freq(:,1))/4*100;
exploit_LH_perc_nosim = (chosenOption.ABD.freq(:,4)+chosenOption.AB.freq(:,4)+chosenOption.BD.freq(:,4)+chosenOption.AD.freq(:,4))/4*100;
...
%     [nanmean(exploit_SH_perc_nosim(idx_prop)) nanmean(exploit_LH_perc_nosim(idx_prop))], ...
%     [nanstd(exploit_SH_perc_nosim(idx_prop))./sqrt(numel(idx_prop)) nanstd(exploit_LH_perc_nosim(idx_prop))./sqrt(numel(idx_prop))],'.','color',col.prop(2,:), 'MarkerSize',6, 'MarkerEdgeColor',col_sim,'MarkerFaceColor',col_sim);
% set(h,'Marker','d')
% 
% h = errorbar(x_ax([4 5])+0.25,...
%     [nanmean(exploit_SH_perc_nosim(idx_plc)) nanmean(exploit_LH_perc_nosim(idx_plc))], ...
%     [nanstd(exploit_SH_perc_nosim(idx_plc))./sqrt(numel(idx_plc)) nanstd(exploit_LH_perc_nosim(idx_plc))./sqrt(numel(idx_plc))],'.','color',col.plac(2,:), 'MarkerSize',6, 'MarkerEdgeColor',col_sim,'MarkerFaceColor',col_sim);
% set(h,'Marker','d')
% 
% h = errorbar(x_ax([7 8])+0.25,...
%     [nanmean(exploit_SH_perc_nosim(idx_ami)) nanmean(exploit_LH_perc_nosim(idx_ami))], ...
%     [nanstd(exploit_SH_perc_nosim(idx_ami))./sqrt(numel_idx_ami) nanstd(exploit_LH_perc_nosim(idx_ami))./sqrt(numel_idx_ami)],'.','color',col.ami(2,:), 'MarkerSize',6, 'MarkerEdgeColor',col_sim,'MarkerFaceColor',col_sim);
% set(h,'Marker','d')
% 
% 
...
export_fig([out_dir 'Fig_behaviour_freq_high_value_bandit_sim_new.tif'],'-nocrop','-r200')
