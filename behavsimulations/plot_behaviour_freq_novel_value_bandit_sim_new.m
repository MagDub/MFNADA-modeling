...
pickedC_SH_perc_nosim = behaviour(:,8)/n_trials_SH*100;
pickedC_LH_perc_nosim = behaviour(:,4)/n_trials_LH*100;
...
% Short horizon
%b1S= bar(x_ax(1),nanmean(pickedC_SH_perc(idx_prop)),'FaceColor',col.prop(1,:), 'FaceAlpha', 0.25, 'BarWidth',.5, 'LineStyle', '--'); 
...
% Long horizon
% b1L = bar(x_ax(2),nanmean(pickedC_LH_perc(idx_prop)),'FaceColor',col.prop(1,:), 'FaceAlpha', 1, 'BarWidth',.5, 'LineStyle', '--');
...
%     [nanmean(pickedC_SH_perc_nosim(idx_prop)) nanmean(pickedC_LH_perc_nosim(idx_prop))], ...
%     [nanstd(pickedC_SH_perc_nosim(idx_prop))./sqrt(numel(idx_prop)) nanstd(pickedC_LH_perc_nosim(idx_prop))./sqrt(numel(idx_prop))],'.','color',col.prop(2,:), 'MarkerSize',6, 'MarkerEdgeColor',col_sim,'MarkerFaceColor',col_sim);
% set(h,'Marker','d')
% 
% h = errorbar(x_ax([4 5])+0.25,...
%     [nanmean(pickedC_SH_perc_nosim(idx_plc)) nanmean(pickedC_LH_perc_nosim(idx_plc))], ...
%     [nanstd(pickedC_SH_perc_nosim(idx_plc))./sqrt(numel(idx_plc)) nanstd(pickedC_LH_perc_nosim(idx_plc))./sqrt(numel(idx_plc))],'.','color',col.plac(2,:), 'MarkerSize',6, 'MarkerEdgeColor',col_sim,'MarkerFaceColor',col_sim);
% set(h,'Marker','d')
% 
% h = errorbar(x_ax([7 8])+0.25,...
%     [nanmean(pickedC_SH_perc_nosim(idx_ami)) nanmean(pickedC_LH_perc_nosim(idx_ami))], ...
%     [nanstd(pickedC_SH_perc_nosim(idx_ami))./sqrt(numel_idx_ami) nanstd(pickedC_LH_perc_nosim(idx_ami))./sqrt(numel_idx_ami)],'.','color',col.ami(2,:), 'MarkerSize',6, 'MarkerEdgeColor',col_sim,'MarkerFaceColor',col_sim);
% set(h,'Marker','d')
% 
% 
...
export_fig([out_dir 'Fig_behaviour_freq_novel_value_bandit_sim_new.tif'],'-nocrop','-r200')
