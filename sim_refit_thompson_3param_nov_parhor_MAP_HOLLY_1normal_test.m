% function sim_refit_thompson_3param_nov_parhor_MAP_HOLLY_1normal_test(ID)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    settings.funs.valuefun      = @mvnorm_Thompson_noveltybonus_new; %test; %@hybrid; % @thompson; % 
    ...
    if holly == 1
        sim_dir = strcat('/home/mdubois/data/simulation_data/thompson/parhor_MAP/', int2str(n_per_dim), 'perdim_newbounds_sgm0_Q0/');
    else
        sim_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\thompson\parhor_MAP\', int2str(n_per_dim), 'perdim_newbounds_sgm0_Q0\');
    end
...
%                     if c==2 && mo.mat.unshown_tree{c,g}==3
%                         disp(mo.mat.pi{c,g})
%                     end
...
%     disp(mo.mat.sgm{1,1}(:,1))
%     disp(mo.mat.sgm{2,1}(:,1))
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/simulation_data/thompson/parhor_MAP/', int2str(n_per_dim), 'perdim_newbounds_sgm0_Q0/results/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\thompson\parhor_MAP\', int2str(n_per_dim), 'perdim_newbounds_sgm0_Q0\results\');
    end
    ...
    %% prior
    if holly == 1
        load('/home/mdubois/data/MaggiesFarm/participant_data/priors/thompson_4params_sgm0_xi_eta_uni/empirical_prior.mat'); %from generate_empirical_priors.mend
    else
...
%               xo_fmincon = (b-a).*rand(1,size(mEmatparams,2)) + a; % random value in this interval
%                xo_fmincon = (b-a).*rand(1,7) + a; % random value in this interval
