function sim_refit_hybrid_6param(ID)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    settings.desc = ['sim_hybrid_' int2str(ID)];    % description of model (settings, etc)
    settings.params.param_names = {'gamma',                  'tau'        'xi'    'sgm0'    'Q0'    'w_hyb'};   % is same param name as prev, write ''
    settings.params.lb          = [param_bounds_gamma(1)    param_bounds_tau(1) param_bounds_xi(1) param_bounds_sgm0(1)    param_bounds_Q0(1)   param_bounds_w_hyb(1)];    % lower bound
    settings.params.ub          = [param_bounds_gamma(2)    param_bounds_tau(2) param_bounds_xi(2) param_bounds_sgm0(2)    param_bounds_Q0(2)   param_bounds_w_hyb(2)];    % upper bound
    
    if holly == 1
        sim_dir = strcat('/home/mdubois/data/simulation_data/hybrid/6params/', int2str(n_per_dim), 'perdim/');
    else
        sim_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\hybrid\6params\', int2str(n_per_dim), 'perdim\');
    end

    %% load parameter values
    tmp = load([sim_dir 'inp_params_hybrid.mat']);
    ...
    for c = 1 %:settings.task.N_hor
        for g = 1:settings.task.Ngames_per_hor         
            ...
            for t = 1:size(tmp_dat.alltrees,1)+1 %CHANGED Added one because [Q0 Q]
    ...
    if holly == 1
        results_dir = strcat('/home/mdubois/data/simulation_data/hybrid/6params/', int2str(n_per_dim), 'perdim/results/');
    else
        results_dir = strcat('D:\MaggiesFarm\modeling_28_02\simulation_data\hybrid\6params\', int2str(n_per_dim), 'perdim\results\');
    end
...
%     toc
       ...
       mEmatparams = nan(100,6);
       mEmatmle = nan(100,1);
       mEexitflag = nan(100,1);
       
            parfor iter=1:100%:20%:100%:2 %:3%:20 %:100
                disp(iter)
            % starting point
            xo_fmincon = (b-a).*rand(1,1) + a; % random value in this interval     
                [mEparams, mEmle, mEexitflag] = fmincon(modelfun,...
