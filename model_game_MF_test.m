...
%     disp(strcat('in model game MF test, sigma_0(1)=', 32, num2str(mo.params.sgm0(1))))
%     disp(strcat('in model game MF test, sigma_02=', 32, num2str(mo.params.sgm02)))
    ...
    if t == 1 % plug in priors
        ...
        mo.mat.sgm{idx_hor,idx_g}(:,t) = mo.params.sgm0(data.gameNo);
            ...
            disp(strcat('horizon = ',32,  num2str(idx_hor)))
        disp(strcat('in model game MF test, sigma(1)=', 32, num2str(mo.mat.sgm{idx_hor,idx_g}(:,1))))
        disp(strcat('in model game MF test, sigma(1)=', 32, num2str(mo.mat.sgm{idx_hor,idx_g}(:,1))))
        disp(strcat('in model game MF test, sigma_02(1)=', 32, num2str(mo.params.sgm02(1))))
        disp(strcat('in model game MF test, sigma_02(2)=', 32, num2str(mo.params.sgm02(2))))
    end
