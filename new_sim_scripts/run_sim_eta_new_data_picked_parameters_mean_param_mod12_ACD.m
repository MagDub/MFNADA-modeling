...
sim_folder = strcat('D:\writing\MF\data_for_figs\sim_prob_mod12_changing_eta_ACD\sgm0_',num2str(sgm0_mat),'_Q0_',num2str(Q0_mat),'_xi_',num2str(xi_mat),'\');
...
for eta_iter = 1:size(eta_range,2)
    ...
    eta = [eta_range(eta_iter) 0.5+eta_range(eta_iter)];

    xi = [xi_mat, xi_mat];

    para_vals_desc = {'sgm0_SH', 'sgm0_LH', 'Q', 'eta_SH', 'eta_LH', 'xi_SH', 'xi_LH'};
    ...
    for ID = 1:part_num
        ...
        settings.params.param_names = {'sgm0', '', 'Q0','eta', '', 'xi', ''};   % is same param name as prev, write ''
    ...
    for c = 1:settings.task.N_hor
                ...
                exploitTree(g,c)=1;
                    ...
                    if t <= size(tmp_dat.alltrees,1) %CHANGED
                        ...
                        mo = mo.funs.valuefun(mo,c,g,t);
                            ...
                            % add lapse if needed
                            if ~isempty(mo.params.eta) %eta greedy
                                mo = lapse(mo,c,g,t);
                            end
                    end
                end
        ...
        [sim_consistency_freq, sim_consistency_freq_desc] = make_consistency_from_sim_percond_ACD(sim_data_dir);
