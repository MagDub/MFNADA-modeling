...
function [] = run_sim_mod5_changing_gamma_xi(tau, xi, Q0, gamma_range)

    part_num = 200;

    sim_folder = strcat('../../../data/simulation_data/sim_prob_mod5_xi/changing_gamma/xi_', num2str(xi), '_tau_',num2str(tau),'_Q0_',num2str(Q0),'/');

    addpath('../../fit/holly/')

    for gamma_iter = 1:size(gamma_range,2)

        gamma = gamma_range(gamma_iter);

        para_vals_desc = {'tau', 'xi', 'Q0', 'gamma'};

        para_vals = [tau, xi, Q0, gamma]; 
        ...
        for ID = 1:part_num
            ...
            gamma_file = strcat('gamma_',num2str(gamma_range(gamma_iter)));

            sim_data_dir = strcat(sim_folder,gamma_file,'/participant_',int2str(ID),'/');
        ...
        mat_mean_SEM_consistency(gamma_iter,:) = [gamma mean(mat_consistency) std(mat_consistency)/sqrt(part_num-1)];

        mat_mean_SEM_tree_high(gamma_iter,:) = [gamma mean(mat_trees(:,1)) std(mat_trees(:,1))/sqrt(part_num-1)];
        mat_mean_SEM_tree_medium(gamma_iter,:) = [gamma mean(mat_trees(:,2)) std(mat_trees(:,2))/sqrt(part_num-1)];
        mat_mean_SEM_tree_novel(gamma_iter,:) = [gamma mean(mat_trees(:,3)) std(mat_trees(:,3))/sqrt(part_num-1)];
        mat_mean_SEM_tree_low(gamma_iter,:) = [gamma mean(mat_trees(:,4)) std(mat_trees(:,4))/sqrt(part_num-1)];

        mat_mean_SEM_tree_standard_certain(gamma_iter,:) = [gamma mean(mat_trees_bandits(:,1)) std(mat_trees_bandits(:,1))/sqrt(part_num-1)];
        mat_mean_SEM_tree_standard(gamma_iter,:) = [gamma mean(mat_trees_bandits(:,2)) std(mat_trees_bandits(:,2))/sqrt(part_num-1)];
